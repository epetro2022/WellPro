...
{
    public partial class BeamWellGroupConfiguration : DevExpress.XtraEditors.XtraUserControl
    {
        DB dbCon;
        public BeamWellGroupConfiguration()
        {
            InitializeComponent();
            dbCon = new DB(Globals.getstringco());
        }

        private void BeamWellGroupConfiguration_Load(object sender, EventArgs e)
        {
            dbCon.clearCache();
            Dictionary<string, object> paras = new Dictionary<string, object>();
            paras.Add("@param", "");
            DataSet ds = dbCon.execprosedure("dbo.[prosedur_getdataBeamWellGroupConfiguration]", paras);
